/* ============================================================
   ...
   ============================================================ */
import { createPanZoom } from './panzoom.js?v=15';
import { openModal, closeModal, escapeHtml } from './modal.js?v=15';
import { openSystem, slugify } from './system-view.js?v=15';
import { openPhenom } from './phenom.js?v=15';
import { openStory } from './stories.js?v=15';
...
(async function(){
  ...
  (function addOuterMask(){
    ...
    /* ПОДКРУТИТЬ РУКАМИ: сдвиг границы маски относительно точного края картинки,
       в единицах SVG (та же система координат, что и у SYSTEMS/калибровки выше).
       Положительное число — маска отступает НАРУЖУ (открывает больше настоящей карты
       по этому краю, значит меньше отрезает). Отрицательное — маска заходит ВНУТРЬ
       (прячет больше, если с этой стороны всё ещё вылезают "NAME_..." подписи). */
    const MASK_ADJUST = {
      top: -2,
      bottom: 3,
      left: -2,
  ...
  const setupSystemLabels = (readySlugs) => {
    svg.querySelectorAll('text').forEach(textEl => {
      ...
      // по этому признаку их и отсеиваем, кликабельными оставляем только системы.
      ...
      // Подсветка систем, для которых уже есть готовая карта — просто перекрашиваем
      // сам текст, никаких новых DOM-элементов не добавляем, ноль лишней нагрузки.
      const slug = slugify(name);
      if (readySlugs.has(slug)) {
        textEl.setAttribute('fill', '#AFEEEE');
      }
